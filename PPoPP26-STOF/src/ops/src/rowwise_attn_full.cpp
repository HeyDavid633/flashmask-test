...
void rowwise_attn_full_gpu(at::Tensor q, at::Tensor k, at::Tensor v, bool is_causal, at::Tensor result)
{
    ...
    const int stride_0 = q.stride(0);
    const int stride_1 = q.stride(1);
    const int stride_2 = q.stride(2);

    launcher_rowwise_attn_full(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def("forward", &rowwise_attn_full_gpu, "launcher_rowwise_attn_full"); 
} 