...
parser.add_argument("--input_file", type=str, default="3-moti-4090_3_2.csv", help="Path to the input CSV file")
...
for idx, op in enumerate(operators):
    ...
    # 使用自定义标签而不是自动生成的标签
    # 确保自定义标签的数量与数据点数量匹配
    labels = custom_labels[idx][:len(op_data)]  # 只取需要的标签数量
    ...
    # 使用自定义标签
    axes[idx].set_xticklabels(labels, fontname='Times New Roman', fontsize=25)  # 减小字体大小
    axes[idx].tick_params(axis='x', labelsize=22, pad=2)  # 减小标签大小

    add_labels_with_cutoff(rects2, axes[idx], y_cutoff)

    axes[idx].set_ylim(0, y_cutoff)
    axes[idx].set_yticks(np.arange(0, y_cutoff + 1, 1))
    # axes[idx].tick_params(axis='both', labelsize=22)
