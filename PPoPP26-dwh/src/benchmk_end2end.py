    ...
    with torch.no_grad():
        ...
        for layer in range(layer_num):
            ...
            if attention_type=="torch_attention":
                ...
                _, h = mcfuser_attn(q1, k1, v1, mask, batch_size, head_num, seq_len, head_size)
                h = h.permute(0, 2, 1, 3) 
                new_context_layer_shape = h.size()[:-2] + (hidden_dim, )
                hidden_states = h.reshape(new_context_layer_shape)
# 
            ...
            # print(f"output_kernel device: {output_kernel[layer].device}")
            # print(f"output_bias device: {output_bias[layer].device}")
            # print(f"hidden_states shape: {hidden_states.shape}")
            # print(f"output_kernel shape: {output_kernel[layer].shape}")
            # print(f"output_bias shape: {output_bias[layer].shape}")
            # hidden_states = torch.matmul(hidden_states.to(q.device), output_kernel[layer].to(q.device)) + output_bias[layer].to(q.device)
...
def llama3_base_fwd_std(mask):
    ...
    for layer in range(layer_num):
        ...
        # print(f"mask shape: {inner_bitmaps.dtype}")
        # ------------------------------------------------------------- Attention start
        if attention_type=="torch_attention":
            q = rearrange(q, 'b t h d -> (b h) t d')
            k = rearrange(k, 'b s h d -> (b h) d s')
...
def vit_base_fwd_std(mask):
    ...
    for layer in range(layer_num):
        ...
        # query_layer = nn.Linear(hidden_size, hidden_size, bias=True).to(device=device)
        # key_layer = nn.Linear(hidden_size, hidden_size, bias=True).to(device=device)
        # value_layer = nn.Linear(hidden_size, hidden_size, bias=True).to(device=device)
        
        # q = query_layer(hidden_states)  
        # k   = key_layer(hidden_states)      
        # v = value_layer(hidden_states)  
        
        # q = q.view(batch_size, seq_len, head_num, head_size)
        # k = k.view(batch_size, seq_len, head_num, head_size)
        # v = v.view(batch_size, seq_len, head_num, head_size)
        qkv = hidden_states 
        # q, k, v = qkv.chunk(3, dim=-1)
        ...
        if attention_type=="torch_attention":
            ...
            # print(f"v shape: {v.shape}, dtype: {v.dtype}")
            # print(f"mask shape: {inner_bitmaps.dtype}")
            # inner_bitmaps=inner_bitmaps.half()
            h = binding_attn_func(q, k, v,
                            full_row_ptr, full_col_idx, 
                            part_row_ptr, part_col_idx, inner_bitmaps, 
                            load_row_ptr, load_col_idx, 
        ...
        #   h = h.float()    
            ...
            _, h = mcfuser_attn(q1, k1, v1, vit_mask, batch_size, head_num, seq_len, head_size)
            h=h.permute(0, 2, 1, 3) 
            new_context_layer_shape = h.size()[:-2] + (hidden_dim, )
...
if __name__ == "__main__":
    ...
    if(test_Torch):
        ...
        for i in range(warmup_iters + running_iters):
            ...
            # llama3_base_fwd_std(mask)
            # vit_base_fwd_std(vit_mask)
            # print(mask.size())
