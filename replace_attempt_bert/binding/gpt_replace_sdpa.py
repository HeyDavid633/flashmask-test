...
#  
# 有了 bert_replace_sdpa.py 成功经验后，将 GPT 中的 SDPA替换为我们的
import sys
...
def patch_gpt2_attention_no_kv_cache():
    from transformers.models.gpt2.modeling_gpt2 import GPT2Attention
    origin_forward = GPT2Attention.forward  # 保存原始方法
    ...
    def patched_forward(
        ...
        use_cache=False,            # 固定为 False
        output_attentions=False,
        **kwargs,
    ):
        # 强制 use_cache=False，忽略 layer_past
        use_cache = False
        layer_past = None

        # 直接调用原始 forward，但跳过 present 返回逻辑
        outputs = origin_forward(
            self,
        ...
        # 返回 only attn_output，不要 present
        return (outputs[0],) if len(outputs) > 1 else outputs

    # 替换方法
    GPT2Attention.forward = patched_forward

if __name__ == '__main__':
    patch_gpt2_attention_no_kv_cache()
    ...
    config = model.config
    batch_size, seqlen, nheads, headdim = inputs["inpunt_ids"].shape[0], inputs["input_ids"].shape[1], config.num_attention_heads, config.hidden_size // config.num_attention_heads
