...
for MASK in "${mask_id[@]}"; do
    ...
    timestamp=$(date +"%m%d_t%H%M")
    output_file="$output_dir/$running_device/d${timestamp}_results_${mask_type}_onlyFA2.txt"
    ...
    for bs in "${batch_sizes[@]}"; do
        ...
        for seq in "${seq_lengths[@]}"; do
            ...
            # 执行测试并捕获输出
            result=$(python benchmark_onlyFA2.py --mask_id $MASK --batch_size $bs --seq_len $seq 2>&1)
