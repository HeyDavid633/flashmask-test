...
// 在 reduce.h 中添加以下代码   rmf
__device__ __forceinline__ __half2& operator+=(__half2& a, const __half2& b) {
  a = __hadd2(a, b);
  return a;
}
...
__inline__ __device__ __half blockReduceSum(__half2 val) {
  ...
  return __float2half(wid == 0 ? warpReduceSum(threadIdx.x < (blockDim.x >> 5)
                                              //  ? (float)__half2add(shared[lane])
                                              ? __half2float(__half2add(shared[lane]))  //rmf
                                               : 0.0f)
                           : 0.0f);
}
